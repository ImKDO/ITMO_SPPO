package org.example.AskManager;
...
public class Ask {
    ...
    public Ask(Console console, String login) {
    ...
    public HumanBeing askHumanBeing(int id) {
        if (console.getFileScanner() == null) {
            ...
            console.println(hasToothpick);
            return new HumanBeing(login, id, name, new Coordinates(x, y), realHero, hasToothpick, impactSpeed, LocalDateTime.now(), soundtrackName, weaponType, mood, new Car(nameCar, coolCar));
        } else {
            String name;
            while (true) {
                var line = console.readln().trim();
