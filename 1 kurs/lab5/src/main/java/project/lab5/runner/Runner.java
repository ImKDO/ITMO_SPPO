...
public class Runner {
    ...
    private ExecutionResponse scriptMode(String argument) {
        ...
        if (!new File("test_execute_script.txt").exists()) return new ExecutionResponse(false, "Файл не существет!");
        if (!Files.isReadable(Paths.get("test_execute_script.txt"))) return new ExecutionResponse(false, "Прав для чтения нет!");
    ...
    private ExecutionResponse launchCommand(String[] userCommand) throws Ask.AskBreak {
        ...
        switch (userCommand[0]) {
            case "execute_script" -> {
                ExecutionResponse tmp = commandManager.getCommands().get("execute_script").apply(userCommand[0]);
            ...
            default -> { return command.apply(userCommand[0]); }
        }
    }
}
